...
const testResults = new RegExp(
  "Tests: ([0-9]+), Assertions: ([0-9]+)(, Errors: ([0-9]+))?(, Failures: ([0-9]+))?(, Skipped: ([0-9]+))?(, Incomplete: ([0-9]+))?."
);
const testResultsOk = new RegExp(
  "OK \\(([0-9]+) tests, ([0-9]+) assertions\\)"
);

const missingPackage = new RegExp(
  "The requested package (?<library>[^ ]+) could not be found in any version"
);
const missingPackage2 = new RegExp(
  'No releases available for package "(?<library>[^"]+)"'
);
