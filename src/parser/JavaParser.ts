...
const independents = [
  ...
  {
    element: new RegExp(
      "(?<nbTest>[0-9]+) tests completed, (?<failed>[0-9]+) failed, ((?<skipped>[0-9]+) skipped)?"
    ),
    type: "test",
    failure_group: "Test",
  },
  {
    element: new RegExp(
      new RegExp(
        " (?<name>[a-zA-Z0-9\\-_]+)\\(\\) (?<status>↷|■|✔)( (?<message>.*))?"
      )
    ),
    type: "test",
    failure_group: "Test",
  ...
  {
    element: new RegExp(
      "(?<status>Failed) test (?<name>.+) \\[(?<class>.+)\\] with exception: "
    ),
    type: "test",
    failure_group: "Test",
  },
  {
    element: new RegExp(
      "\\[javac\\] (?<file>[^:]+):(?<line>[0-9]+): error: (?<message>.*)"
    ),
    type: "Compilation",
  ...
  {
    element: new RegExp(
      "\\[ERROR\\] (?<file>[^:]+):\\[(?<line>[0-9]+)(,(?<column>[0-9]+))?\\] (?<message>\\((.+)\\) (.+))."
    ),
    type: "Checkstyle",
    failure_group: "Chore",
  },
  {
    element: new RegExp(
      "\\[checkstyle\\]( \\[ERROR\\])? (?<file>[^:]+):(?<line>[0-9]+):((?<column>[0-9]+):)? (?<message>.+)"
    ),
    type: "Checkstyle",
    failure_group: "Chore",
  },
  {
    element: new RegExp(
      "Could not find (?<group>[^: ]+):(?<artifact>[^: ]+)(:(pom|jar))?:(?<version>[^ ]+)."
    ),
  ...
  {
    element: new RegExp(
      "Could not transfer artifact (?<group>[^: ]+):(?<artifact>[^: ]+)(:(pom|jar))?:(?<version>[^ ]+)"
    ),
    type: "Missing library",
    failure_group: "Installation",
  },
  {
    element: new RegExp(
      "Failure to find (?<group>[^: ]+):(?<artifact>[^: ]+)(:(pom|jar))?:(?<version>[^ ]+)"
    ),
    type: "Missing library",
    failure_group: "Installation",
  },
  {
    element: new RegExp(
      "PMD Failure: (?<file>[^:]+):(?<line>[0-9]+) Rule:(?<rule>.+) Priority:(?<priority>[0-9]+) (?<message>.+)"
    ),
    type: "Checkstyle",
    failure_group: "Chore",
  },
  {
    element: new RegExp(
      "(?<nbTest>[0-9]+) tests? completed, (?<failure>[0-9]+) failed"
    ),
    type: "test",
    failure_group: "Test",
  },
...
const groups = [
  ...
  },
  {
    ...
    element: new RegExp(
      "(?<file>[^:]+):\\[(?<line>[0-9]+)(,(?<column>[0-9]+))?\\] error: (?<message>.+)"
    ),
  },
  {
    name: "compile",
  ...
  },
  {
    ...
    start: new RegExp("([0-9]+)\\) (?<name>.+) \\((?<class>.+)\\)"),
      ...
      "Tests run: (?<nbTest>[0-9]+),  Failures: (?<failure>[0-9]+)"
    ),
    element: new RegExp("(?<all_line>.+)"),
  },
  {
    ...
    start: new RegExp("Executing test (?<name>.+) \\[(?<class>.+)\\]"),
      ...
      "(((?<nbTest>[0-9]+) tests completed, (?<failure>[0-9]+) failed)|(Executing test (?<name>.+) \\[(?<class>.+)\\]))"
    ),
    startIsEnd: true,
    element: new RegExp("(?<all_line>.+)"),
  },
  ...
  parse(line: string, lineNumber: number) {
      ...
      if (this.inGroup == null) {
        ...
        if (result != null) {
          if (result.groups.all_line) {
            ...
            this.errors.push(output as any);
          }
          return;
        }
      }
    ...
    for (const independent of independents) {
      ...
      if (result != null) {
        ...
        for (let key in result.groups) {
          if (result.groups[key] != null) {
            let value: string | number = result.groups[key];
